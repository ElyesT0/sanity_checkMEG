...
def load_behavioral_file(subject, start_keyword='subject_id'):
    ...
    """
    file_path=path_processed_behavioral_file
...
def extract_epochs_first_presentation(subject):
    ...
    # todo for Elyès
    path=epo_item_path
    # path = derivatives_path + '/items/sub-'+subject+'/meg/sub-'+subject+'_task-reproduction_epo.fif'
    ...
    expanded_metadata=expand_dataframe_with_position(metadata)
    print('np.shape(epochs) : ',np.shape(epochs))
    epochs.metadata = expanded_metadata
